...
static void relay(int fd1, int fd2)
{
    ...
    while (fsm1.state != STATE_T && fsm2.state != STATE_T)
    {
        ...
        if (fsm1.state == STATE_R)
            FD_SET(fsm1.fd_src, &rset);
        if (fsm2.state == STATE_R)
            FD_SET(fsm2.fd_src, &rset);
        if (fsm1.state == STATE_W)
            FD_SET(fsm1.fd_dst, &wset);
        if (fsm2.state == STATE_W)
            FD_SET(fsm2.fd_dst, &wset);

        /* monitor */
        /* !!! select在监测普通文件时，无论是可读、可写还是警告，其一直会返回准备就绪状态 */
        if (select(max_value(fd1, fd2)+1, &rset, &wset, NULL, NULL) < 0)
        {
            if (errno == EINTR)
                continue;
            perror("select()");
        ...
        if (FD_ISSET(fsm1.fd_src, &rset) || FD_ISSET(fsm1.fd_dst, &wset))
            driver(&fsm1);
        if (FD_ISSET(fsm2.fd_src, &rset) || FD_ISSET(fsm2.fd_dst, &wset))
            driver(&fsm2);
    }
